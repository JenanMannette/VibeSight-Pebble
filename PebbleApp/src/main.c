...
static void prv_update_text(void) {
  if (smartstrap_service_is_available(SMARTSTRAP_RAW_DATA_SERVICE_ID)) {
    text_layer_set_text(s_status_layer, "Hooray!");
  } else {
    text_layer_set_text(s_status_layer, "Connecting...");
  }
}

// Vibe pattern: ON for 50ms, OFF for 800ms, ON for 50ms:
static const uint32_t const segments[] = { 50, 800, 50 };
VibePattern pat = {
    .durations = segments,
    .num_segments = ARRAY_LENGTH(segments),
};

// Vibe pattern: ON for 50ms, OFF for 600ms, ON for 50ms:
static const uint32_t const segments2[] = { 50, 600, 50 };
VibePattern pat2 = {
    .durations = segments2,
    .num_segments = ARRAY_LENGTH(segments2),
};

// Vibe pattern: ON for 50ms, OFF for 400ms, ON for 50ms:
static const uint32_t const segments3[] = { 50, 400, 50 };
VibePattern pat3 = {
    .durations = segments3,
    .num_segments = ARRAY_LENGTH(segments3),
};

// Vibe pattern: ON for 50ms, OFF for 100ms, ON for 50ms:
static const uint32_t const segments4[] = { 50, 100, 50 };
VibePattern pat4 = {
    .durations = segments4,
    .num_segments = ARRAY_LENGTH(segments4),
};

// Vibe pattern: ON for 50ms, OFF for 50ms, ON for 50ms:
static const uint32_t const segments5[] = { 50, 50, 50 };
VibePattern pat5 = {
    .durations = segments5,
...
static void prv_send_request(void *context) {
  ...
  prv_read_raw();
  app_timer_register(200, prv_send_request, NULL);
}
